...
#define reset(pin)	1<<(pin+16)
#define set(pin)		1<<pin

//GLCD Function
void glcd_putchar(char data,unsigned char j);
...
void glcd_disp_Image(char *str,char x1,char x2, char y1,char y2);
...
void send_command(unsigned char command){
	Delay(T);
	CTRL_PORT->BSRR = reset(E_PIN_NUMBER);      //E_PIN_NUMBER=RESET
	Delay(T);
	CTRL_PORT->BSRR = reset(RW_PIN_NUMBER);     //RW_PIN_NUMBER=RESET
	Delay(T);
	CTRL_PORT->BSRR = reset(DI_PIN_NUMBER);     //DI_PIN_NUMBER=RESET
	Delay(T);
	COL_PORT->BSRR = set(CS1_PIN_NUMBER);         //CS1_PIN_NUMBER=SET
	Delay(T);
	COL_PORT->BSRR = set(CS2_PIN_NUMBER);         //CS2_PIN_NUMBER=SET
	Delay(T);
  DATA_PORT->BSRR = 0xFF << (D0_PIN_NUMBER+16);  //DATA_PORT=RESET
	Delay(T);
	CTRL_PORT->BSRR = set(E_PIN_NUMBER);           //E_PIN_NUMBER=SET
	Delay(T);
	DATA_PORT->BSRR = command << D0_PIN_NUMBER;    //DATA_PORT=Command
	Delay(T);
	CTRL_PORT->BSRR = reset(E_PIN_NUMBER);      //E_PIN_NUMBER=RESET
}
//--------------------------------------
...
//--------------------------------------
void glcd_putchar(char data,unsigned char j){ //print one char on glcd
	if(j<64){                                      //Left section
		CTRL_PORT->BSRR = 1 << (E_PIN_NUMBER+16);      //E_PIN_NUMBER=RESET
		Delay(T);
		CTRL_PORT->BSRR = 1 << (RW_PIN_NUMBER+16);     //RW_PIN_NUMBER=RESET
		Delay(T);
		CTRL_PORT->BSRR = 1 << DI_PIN_NUMBER;          //DI_PIN_NUMBER=SET
		Delay(T);
		COL_PORT->BSRR = 1 << CS1_PIN_NUMBER;         //CS1_PIN_NUMBER=SET
		Delay(T);
		COL_PORT->BSRR = 1 << (CS2_PIN_NUMBER+16);    //CS2_PIN_NUMBER=RESET
		Delay(T);
		DATA_PORT->BSRR = 0xFF << (D0_PIN_NUMBER+16);  //DATA_PORT=RESET
		Delay(T);
		CTRL_PORT->BSRR = 1 << E_PIN_NUMBER;           //E_PIN_NUMBER=SET
		Delay(T);
		DATA_PORT->BSRR = data << D0_PIN_NUMBER;       //DATA_PORT=DATA
		Delay(T);
		CTRL_PORT->BSRR = 1 << (E_PIN_NUMBER+16);      //E_PIN_NUMBER=RESET
	}
	else{                                          //Right section
		...
		CTRL_PORT->BSRR = 1 << DI_PIN_NUMBER;          //DI_PIN_NUMBER=SET
		Delay(T);
		COL_PORT->BSRR = 1 << (CS1_PIN_NUMBER+16);    //CS1_PIN_NUMBER=RESET
		Delay(T);
		COL_PORT->BSRR = 1 << CS2_PIN_NUMBER;         //CS2_PIN_NUMBER=SET
		Delay(T);
		DATA_PORT->BSRR = 0xFF << (D0_PIN_NUMBER+16);  //DATA_PORT=RESET
		Delay(T);
		CTRL_PORT->BSRR = 1 << E_PIN_NUMBER;           //E_PIN_NUMBER=SET
...
void goto_column(unsigned char x){
   ...
   if(x<64){		                                       //Left section
   	  x = (x | 0x40 ) & 0xFF;                          //Set command to Display Control Instruction table
		  CTRL_PORT->BSRR = 1 << (E_PIN_NUMBER+16);        //E_PIN_NUMBER=RESET
			Delay(T);
    	CTRL_PORT->BSRR = 1 << (RW_PIN_NUMBER+16);       //RW_PIN_NUMBER=RESET
			Delay(T);
	    CTRL_PORT->BSRR = 1 << (DI_PIN_NUMBER+16);       //DI_PIN_NUMBER=RESET  
			Delay(T);
      COL_PORT->BSRR = 1 << (CS1_PIN_NUMBER);         //CS1_PIN_NUMBER=SET
			Delay(T);
      COL_PORT->BSRR = 1 << (CS2_PIN_NUMBER+16);      //CS2_PIN_NUMBER=RESET  
			...
			CTRL_PORT->BSRR = 1 << E_PIN_NUMBER;             //E_PIN_NUMBER=SET
			Delay(T);
			DATA_PORT->BSRR = x << D0_PIN_NUMBER;            //DATA_PORT=x  
			Delay(T);		 
    	CTRL_PORT->BSRR = 1 << (E_PIN_NUMBER+16);        //E_PIN_NUMBER=RESET		 
   }
   else{                                               //Right section
...
//--------------------------------------
void glcd_disp_Image(char *str,char x1,char x2, char y1,char y2){  //print Image on glcd x1:First_x_axis  x2:End_x_axis  y1:First_y_axis   y2:End_y_axis
	Delay(T);
	  unsigned char i,j;    
    for(i=y1;i<=y2;i++)
    {
...
void glcd_clrearline(unsigned line){                   //Clear Line , Line = 0 to 7
    ...
    glcd_gotoxy(0,line);                               //At start of line of left side
	  COL_PORT->BSRR = 1 << CS1_PIN_NUMBER;             //CS1_PIN_NUMBER=SET
		Delay(T);
	  COL_PORT->BSRR = 1 << (CS2_PIN_NUMBER+16);        //CS1_PIN_NUMBER=RESET
